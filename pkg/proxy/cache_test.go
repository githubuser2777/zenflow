...
func TestTotalSizeEnforcement(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		key := cacheKey{path: fmt.Sprintf("key-%d", i)}
...
func TestLRUEvictionOrder(t *testing.T) {
	...
	c.Set(cacheKey{path: "A"}, http.Header{}, oneKB)
	c.Set(cacheKey{path: "B"}, http.Header{}, oneKB)
	c.Set(cacheKey{path: "C"}, http.Header{}, oneKB)

	// Access A — A becomes MRU
	if _, ok := c.Get(cacheKey{path: "A"}); !ok {
	...
	// B should be evicted
	if _, ok := c.Get(cacheKey{path: "B"}); ok {
		t.Errorf("expected B to be evicted")
	}

	// A, C, D should be present
	for _, key := range []cacheKey{{path: "A"}, {path: "C"}, {path: "D"}} {
...
func TestSingleItemExceedsTotalLimit(t *testing.T) {
	...
	c.Set(cacheKey{path: "A"}, http.Header{}, bigBody)
...
func TestUpdateExistingKey(t *testing.T) {
	...
	c.Set(cacheKey{path: "A"}, http.Header{}, oneKB)
	expectedOne := estimateItemSize(cacheKey{path: "A"}, CachedResponse{Headers: []HeaderField{}, Body: oneKB})
	if c.totalSize != expectedOne {
		t.Errorf("expected totalSize %d, got %d", expectedOne, c.totalSize)
	}

	c.Set(cacheKey{path: "A"}, http.Header{}, twoKB)
	expectedTwo := estimateItemSize(cacheKey{path: "A"}, CachedResponse{Headers: []HeaderField{}, Body: twoKB})
	if c.totalSize != expectedTwo {
		t.Errorf("expected totalSize %d after update, got %d", expectedTwo, c.totalSize)
	}

	resp, ok := c.Get(cacheKey{path: "A"})
...
func TestPerItemCapStillWorks(t *testing.T) {
	...
	c.Set(cacheKey{path: "A"}, http.Header{}, sixMB)
...
func TestConcurrency(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(i int) {
			defer wg.Done()
			c.Set(cacheKey{path: fmt.Sprintf("K%d", i)}, http.Header{}, []byte("test"))
			c.Get(cacheKey{path: fmt.Sprintf("K%d", i)})
		}(i)
	}
	wg.Wait()
}
